...
export class PolicesComponent implements OnInit {
  ...
  constructor(private formBuilder:FormBuilder,private router: Router,private policesService:PolicesService){
  ...
  onSubmit(): void {
    ...
    this.policesService.create(formdata).subscribe(response => {
    ...
    title: 'Are you sure?',
      ...
      if (result.value) {
        this.policesService.create(formdata).subscribe(
          (response) => {
            this.response = response;
