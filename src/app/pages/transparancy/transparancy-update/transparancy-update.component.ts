...
@Component({
  selector: 'app-transparancy-update',
  templateUrl: './transparancy-update.component.html',
  styleUrls: ['./transparancy-update.component.css']
})
export class TransparancyUpdateComponent implements OnInit{
  ...
  constructor(private router: Router,private formBuilder:FormBuilder,private transparancyService:TransparancyService,private route: ActivatedRoute,){
  ...
  onSubmit(): void {
    ...
    // this.transparancyService.update(this.id,formdata).subscribe(response => {
    //
    ...
    Swal.fire({
      ...
      if (result.value) {
        this.transparancyService.update(this.id,formdata).subscribe(
          (response) => {
  ...
  view() {
    this.transparancyService.view(this.id).subscribe(
      res => {
