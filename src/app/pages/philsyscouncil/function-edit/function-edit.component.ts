...
export class FunctionEditComponent implements OnInit {
  ...
  onSubmit(): void {
    ...
    // this.functionService.update(this.id,formdata).subscribe(response => {
    //
    ...
    Swal.fire({
      ...
      if (result.value) {
        this.functionService.update(this.id,formdata).subscribe(
          (response) => {
  ...
  view() {
    this.functionService.view(this.id).subscribe(
      res => {
