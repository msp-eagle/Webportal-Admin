...
@Component({
  ...
  styleUrls: ['./resolution-edit.component.css']
})
export class ResolutionEditComponent implements OnInit {
  ...
  constructor(private formBuilder:FormBuilder,private resolutionService:ResolutionService,
  ...
  onSubmit(): void {
    ...
    // this.resolutionService.update(this.id,formdata).subscribe(response => {
    //
    ...
    Swal.fire({
      ...
      if (result.value) {
        this.resolutionService.update(this.id,formdata).subscribe(
          (response) => {
        ...
        this.router.navigate(['/cms/council/resolution'])
      }
  ...
  view() {
    this.resolutionService.view(this.id).subscribe(
      res => {
