...
@Component({
  selector: 'app-philypscheck',
  templateUrl: './philypscheck.component.html',
  styleUrls: ['./philypscheck.component.css']
})
export class PhilypscheckComponent implements OnInit {
  ...
  constructor(private formBuilder:FormBuilder,private philypscheckService:PhilypscheckService,private router: Router
  ){
  ...
  onSubmit(): void {
    ...
    // this.philypscheckService.create(formdata).subscribe(response => {
    //
    ...
    title: 'Are you sure?',
      ...
      if (result.value) {
        this.philypscheckService.create(formdata).subscribe(
          (response) => {
            this.response = response;
