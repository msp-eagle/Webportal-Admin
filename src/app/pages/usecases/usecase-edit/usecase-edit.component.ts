...
export class UsecaseEditComponent implements OnInit {
  ...
  constructor(private router: Router,private formBuilder:FormBuilder,private usecaseService:UsecaseService,private route: ActivatedRoute,){
  ...
  onSubmit(): void {
    ...
    // this.usecaseService.update(this.id,formdata).subscribe(response => {
    //
    ...
    Swal.fire({
      ...
      if (result.value) {
        this.usecaseService.update(this.id,formdata).subscribe(
          (response) => {
        ...
        this.router.navigate(['/cms/use-case'])
      }
  ...
  view() {
    this.usecaseService.view(this.id).subscribe(
      res => {
