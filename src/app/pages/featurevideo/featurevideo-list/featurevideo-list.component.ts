...
@Component({
  selector: 'app-featurevideo-list',
  templateUrl: './featurevideo-list.component.html',
  styleUrls: ['./featurevideo-list.component.css']
})
export class FeaturevideoListComponent {
  maxSize = 5;
  totalItems: number;
  currentPage = 1;
  start: number = 0;
  ...
  constructor(public featurevideoService: FeaturevideoService) {
  ...
  list(id: any) {
    this.featurevideoService.list(id).subscribe(
      res => {
        this.content = res;
  ...
  delete(id: any) {
    ...
    Swal.fire({
      ...
      if (result.value) {
        this.featurevideoService.delete(id)
          .subscribe(res => {
