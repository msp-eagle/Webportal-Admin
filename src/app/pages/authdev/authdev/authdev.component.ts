...
import { AuthdevService } from "../../../service/partners/authdev.service";
import Swal from "sweetalert2";
import { Router } from "@angular/router";

@Component({
  selector: 'app-authdev',
  templateUrl: './authdev.component.html',
  styleUrls: ['./authdev.component.css'],
...
export class AuthdevComponent implements OnInit {
  ...
  constructor(private formBuilder: FormBuilder, private router: Router, private relpartyService: AuthdevService) {
  ...
  onSubmit(): void {
    ...
    // this.relpartyService.create(formdata).subscribe(response => {
    //
    ...
    Swal.fire({
      ...
      if (result.value) {
        this.relpartyService.create(formdata).subscribe(
          (response) => {
            this.response = response;
        ...
        this.router.navigate(['/devauth-list'])
      }
