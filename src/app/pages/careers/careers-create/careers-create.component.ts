...
@Component({
  ...
  templateUrl: './careers-create.component.html',
  styleUrls: ['./careers-create.component.css']
})
export class CareersCreateComponent  implements OnInit{
  ...
  constructor(private formBuilder:FormBuilder,private router: Router,private carrersService:CarrersService){
  ...
  onSubmit(): void {
    ...
    // this.carrersService.create(formdata).subscribe(response => {
    //
    ...
    title: 'Are you sure?',
      ...
      if (result.value) {
        this.carrersService.create(formdata).subscribe(
          (response) => {
            this.response = response;
        ...
        this.router.navigate(['/cms/careers'])
      }
