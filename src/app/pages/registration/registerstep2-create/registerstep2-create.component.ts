...
@Component({
  selector: 'app-registerstep2-create',
  templateUrl: './registerstep2-create.component.html',
  styleUrls: ['./registerstep2-create.component.css']
})
export class Registerstep2CreateComponent implements OnInit{
  public value: string = `
  ...
  constructor(private formBuilder:FormBuilder,private registersterp2Service:Registersterp2Service,private router: Router){
  ...
  onSubmit(): void {
    ...
    // this.registersterp2Service.create(formdata).subscribe(response => {
    //
    ...
    Swal.fire({
      ...
      if (result.value) {
        this.registersterp2Service.create(formdata).subscribe(
          (response) => {
            this.response = response;
