...
@Component({
  selector: 'app-registerstep1-update',
  templateUrl: './registerstep1-update.component.html',
  styleUrls: ['./registerstep1-update.component.css']
})
export class Registerstep1UpdateComponent implements OnInit{
  ...
  constructor(private router: Router,private formBuilder:FormBuilder,private registerstep1Service:Registerstep1Service,private route: ActivatedRoute,){
  ...
  onSubmit(): void {
    ...
    // this.registerstep1Service.update(this.id,formdata).subscribe(response => {
    //
    ...
    Swal.fire({
      ...
      if (result.value) {
        this.registerstep1Service.update(this.id,formdata).subscribe(
          (response) => {
        ...
        this.router.navigate(['/cms/registration-process'])
      }
  ...
  view() {
    this.registerstep1Service.view(this.id).subscribe(
      res => {
