...
@Component({
  selector: 'app-registerstep1-create',
  templateUrl: './registerstep1-create.component.html',
  styleUrls: ['./registerstep1-create.component.css']
})
export class Registerstep1CreateComponent implements OnInit{
  public value: string = `
  ...
  constructor(private formBuilder:FormBuilder,private registerstep1Service:Registerstep1Service,private router: Router){
  ...
  onSubmit(): void {
    ...
    // this.registerstep1Service.create(formdata).subscribe(response => {
    //
    ...
    Swal.fire({
      ...
      if (result.value) {
        this.registerstep1Service.create(formdata).subscribe(
          (response) => {
            this.response = response;
